...
#include <iostream>
 ...
 GLuint loadTexture(const string filename, int &width, int &height) 
 {
   ...
   if (!is_png) {
     std::cout << "File is not a proper png file. Bailing." << std::endl;
     fclose(fp);
     return TEXTURE_LOAD_ERROR;
   }
   ...
   if (!info_ptr) {
     std::cout << "Could not create png info struct. Bailing" << std::endl;
   ...
   if (!image_data) {
     std::cout << "No image data. Bailing." << std::endl;
